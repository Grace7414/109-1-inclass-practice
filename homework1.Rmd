...
```{r ans12}
# Game5Time
Game5Time <- c(Date=NBA1920Final567$game5$date,StartTime=NBA1920Final567$game5$startime,TimeZone=NBA1920Final567$game5$timezne)
```
...
```{r ans13}
G5TimeParsed1 <- paste(NBA1920Final567$game5$date,2020,NBA1920Final567$game5$startime)
G5TimeParsed <- mdy_hm(G5TimeParsed1)
```
...
```{r ans15}
G5TimeParsed2 <- lubridate::minutes(G5TimeParsed )+ 15
ParisLouis <- with_tz(G5TimeParsed2,tzone = "Europe/Paris")
```
...
```{r ans21}
 sumChNumbers <- factor(chNumbers,levels = c("壹","貳","參","肆","伍","陸","柒","捌","玖"))
chNumbers <- as.integer(sumChNumbers)
sum(chNumbers)
```
